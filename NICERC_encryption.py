...
def bruteCesar():	#Brute forces Cesar cypher, checks results against dictionary.
	next_inq = "y"
	
	while next_inq == "y":
		possibleShifts = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z']
		...
		phrase = input("What do you wish to decode?")
		
		for x in range(len(possibleShifts)):
			for y in range(len(possibleShifts)):
				translation = ""
				shift = (x - y+26)%26
				...
				#from here, take file input from dictionary, check your phrase against phrases from the dictionary
